...
bool FosterAudioStartup()
{
	...
	state->audioEngine = ma_malloc(sizeof(ma_engine), NULL);
	...
	if (MA_SUCCESS != ma_resource_manager_init(&resourceManagerConfig, resourceManager)) {
		...
		ma_free(state->audioEngine, NULL);
...
{
	FosterState *state = FosterGetState();
	ma_engine_uninit(state->audioEngine);
	ma_free(state->audioEngine, NULL);
}

float FosterAudioGetVolume()
{
	return ma_engine_get_volume(FosterGetState()->audioEngine);
}

void FosterAudioSetVolume(int index, float value)
{
	ma_engine_set_volume(FosterGetState()->audioEngine, value);
}

int FosterAudioGetChannels()
{
	return ma_engine_get_channels(FosterGetState()->audioEngine);
}

int FosterAudioGetSampleRate()
{
	return ma_engine_get_sample_rate(FosterGetState()->audioEngine);
}

uint64_t FosterAudioGetTimePcmFrames()
{
	return ma_engine_get_time_in_pcm_frames(FosterGetState()->audioEngine);
}

void FosterAudioSetTimePcmFrames(int index, uint64_t value)
{
	ma_engine_set_time_in_pcm_frames(FosterGetState()->audioEngine, value);
}
...
{
	return ma_engine_listener_is_enabled(FosterGetState()->audioEngine, index);
}

void FosterAudioListenerSetEnabled(int index, bool value)
{
	ma_engine_listener_set_enabled(FosterGetState()->audioEngine, index, value);
}

Vector3 FosterAudioListenerGetPosition(int index)
{
	return vec3f_to_Vector3(ma_engine_listener_get_position(FosterGetState()->audioEngine, index));
}

void FosterAudioListenerSetPosition(int index, Vector3 value)
{
	ma_engine_listener_set_position(FosterGetState()->audioEngine, index, value.x, value.y, value.z);
}

Vector3 FosterAudioListenerGetVelocity(int index)
{
	return vec3f_to_Vector3(ma_engine_listener_get_velocity(FosterGetState()->audioEngine, index));
}

void FosterAudioListenerSetVelocity(int index, Vector3 value)
{
	ma_engine_listener_set_velocity(FosterGetState()->audioEngine, index, value.x, value.y, value.z);
}

Vector3 FosterAudioListenerGetDirection(int index)
{
	return vec3f_to_Vector3(ma_engine_listener_get_direction(FosterGetState()->audioEngine, index));
}

void FosterAudioListenerSetDirection(int index, Vector3 value)
{
	ma_engine_listener_set_direction(FosterGetState()->audioEngine, index, value.x, value.y, value.z);
}

FosterSoundCone FosterAudioListenerGetCone(int index)
{
	FosterSoundCone value;
	ma_engine_listener_get_cone(FosterGetState()->audioEngine, index, &value.innerAngleInRadians, &value.outerAngleInRadians, &value.outerGain);
	return value;
}

void FosterAudioListenerSetCone(int index, FosterSoundCone value)
{
	ma_engine_listener_set_cone(FosterGetState()->audioEngine, index, value.innerAngleInRadians, value.outerAngleInRadians, value.outerGain);
}

Vector3 FosterAudioListenerGetWorldUp(int index)
{
	return vec3f_to_Vector3(ma_engine_listener_get_world_up(FosterGetState()->audioEngine, index));
}
